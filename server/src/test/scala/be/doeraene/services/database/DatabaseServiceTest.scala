...
import tables.{DeletedMovie, Image, Movie}
...
class DatabaseServiceTest extends munit.FunSuite with HasTestPower {
  ...
  db.test("I can track deleted movies, without duplicating a re-delivered delete, and clear them once restored") {
    service =>
      val movie = createAMovie(service)
      assertEquals(service.deletedMovies, Vector.empty)

      service.setDeletedInfo(movie.typedId, movie.name)
      assertEquals(service.deletedMovies, Vector(DeletedMovie(movie.id, movie.name)))

      // the deleted-movie projection is at-least-once: a redelivery of the same Deleted event must not duplicate
      // the row (see DatabaseService.setDeletedInfo).
      service.setDeletedInfo(movie.typedId, movie.name)
      assertEquals(service.deletedMovies, Vector(DeletedMovie(movie.id, movie.name)))

      service.clearDeletedInfo(movie.typedId)
      assertEquals(service.deletedMovies, Vector.empty)
  }
