...
/** Generic entry point for sending entity commands from the frontend over a websocket --
  * see `castorwire.Bridge` for how a Command ADT's `replyTo: castor.Actor[R]` fields
  * survive the trip without ever being duplicated into a separate wire ADT.
  *
  * `routers` is the whitelist of entity kinds actually reachable this way; each service
  * that wants to expose (part of) its Command ADT registers one (see
  * `MoviesService.commandRouter`).
  */
...
class CommandRoutes(routers: Seq[CommandRouter])(using castor.Context, cask.util.Logger) extends cask.Routes {
  ...
  def commands() = cask.WsHandler { underlying =>
    ...
    channel.actor(
      ...
      { case cask.Ws.Close(_, _) => bridge.clear() }
    )
  }
